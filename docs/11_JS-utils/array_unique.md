...
Filters out non-unque values from an array or object.

```JavaScript
array_unique(String: path, Object|Array: array): Object|Array
```
...
`array_unique` can be accessed via FrontBx's utility library

```JavaScript
FrontBx._().array_unique();

const [array_unique] = FrontBx.require(['array_unique']).from('_');
```
...
**array_unique** will return `true` if the item exists or `false` if it does not.
...
// true
array_unique('foo[0]', obj);

// false
array_unique('foo[3]', obj);
```

`array_unique` will work on deeply nested Arrays/Objects without having to run validation that parent items exist first.
...
// true
array_unique('foo[0].bar.baz[1]', obj);

//false
array_unique('foo[0].bar.baz[15]', obj);
```
