...
`animate_css` can be accessed via FrontBx's utility library

```JavaScript
FrontBx._().animate_css();

const [animate_css] = FrontBx.require(['animate_css']).from('_');
...
{
    /* Helpers */
    const [$, animate_css, add_event_listener] = FrontBx.import(['$', 'animate_css', 'add_event_listener']).from('_');
