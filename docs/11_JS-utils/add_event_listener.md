...
`add_event_listener` can be accessed via FrontBx's utility library

```JavaScript
FrontBx._().add_event_listener();

const [add_event_listener] = FrontBx.require(['add_event_listener']).from('_');
