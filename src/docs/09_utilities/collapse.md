...
Hubble's Collapse utility offers an easy to use, simple HTML initialized way to toggle collpased content height via a trigger.
...
Hubble fires a few custom events to attach onto when a a collapse trigger is clicked. The `event.detail.state` key will tell you weather the 
