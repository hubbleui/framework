...
Hubble's `Pjax` Component allows you to easily handle Ajax calls in consistent and flexible manner. 
...
Ajax can be accessed via Hubble's container using the `Ajax` key.

```JavaScript
let Ajax = Hubble.Ajax();
```
