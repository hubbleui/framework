...
```JavaScript
animate_css(element: node, Object: options): Object
```
...
`animate_css` can be accessed via Hubble's utility library

```JavaScript
Hubble._().animate_css();

const [animate_css] = Hubble.require(['animate_css']).from('_');
...
| `options` | `Object`      | An object containing CSS properties and animation options. | Yes      | `{duration: 500, easing: ease}` |
...
| <span class="pad">**easing**   <br> **Type:** `String` (default: `ease`) <br> Any animation easing pattern provided in `camelCase`. <br> **Syntax:** `ease[In][Out][Quad|Quart|Quint|Sine|Expo|Circ|Back]`. See [Easings](#easings) for full list of supported values. <br> **Example:** `{easing: 'easeInExpo'}` </span>|
...
| **Example:** <br> <span class="big-code">`animate_css(node, { height: '300px', easing: 'easeInExpo', duration: 1000, callback: () => { console.log('complete')} } );`</span>|

<div class="api-table"></div>
| `animate_css(element, options[property, from, to, duration, easing, callback] ])`   |
|-------------------------------------------------------------------------------------|
| **Description:** Syntax used to animate a single CSS property while explicitly providing a start value. The starting value will be determined by the currently rendered CSS style on the element. In the case that the start value for the property is not explicit set, animate_css will still be able to calculate the rendered value. Works on all values including those set to `auto`, `initial`, `unset`. |
| **element** <br> **Type:** `HTMLElement` <br> The target DOM element to animate.        |
| **options** <br> **Type:** `Object` <br> An object containing CSS properties an animation options. |
...
| <span class="pad">**from** <br> **Type:** `String` (default: `auto`) <br> The CSS start value of the animation as a string. If value is omitted Hubble will compute the existing value based on the element's currently rendered value automatically. Also accepts CSS values `auto` and `initial` <br> **Example:** `{from: '10px'}` </span>|
...
| <span class="pad">**duration** <br> **Type:** `Integer` (default: `500`) <br> The animation duration provided in milliseconds. <br> **Example:** `{duration: 1000}` </span>|
| <span class="pad">**callback** <br> **Type:** `Function` (default: `null`) <br> The CSS property to animate. <br> **Example:** `{callback: () => {} }` </span>|
| **Example:** <br> <span class="big-code">`animate_css(node, { property: 'height', from: '10px', to: '300px', easing: 'easeInExpo', duration: 1000, callback: () => { console.log('complete')} } );`</span>|
...
| <span class="pad"><span class="pad">**easing**   <br> **Type:** `String` (default: `ease`) <br> Any animation easing pattern provided in `camelCase`. <br> **Syntax:** `ease[In][Out][Quad|Quart|Quint|Sine|Expo|Circ|Back]`. See [Easings](#easings) for full list of supported values. <br> **Example:** `{easing: 'easeInExpo'}` </span></span>|
...
| **Example:** <br> <span class="big-code">`animate_css(node, { height:{ from: '0px', to: '100px', easing: 'easeInOutCirc'}, opacity:{ to: 0, easing: 'linear'} } );;`</span>|
...
```JavaScript
animate_css($('.js-animate-example'), {
    width: 
    {
        to : '500px',
        duration: 1000,
...
{
    /* Helpers */
    const [$, animate_css, add_event_listener] = Hubble.import(['$', 'animate_css', 'add_event_listener']).from('_');
