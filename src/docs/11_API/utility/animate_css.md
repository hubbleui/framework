...
| **Example:** <br> <span class="big-code">`animate_css(node, { height:{ from: '0px', to: '100px', easing: 'easeInOutElastic'}, opacity:{ to: 0, easing: 'linear'} } );;`</span>|
...
When animating numerical values that require a unit (e.g. `20px`), animate_css will convert any non **px** units to to **px**, including any existing start values if not provided explicitly. animate_css supports all CSS units such as `%`, `em`, `rem` etc...
...
animate_css will always return and `Animation` Object. Which have two available methods: `start`,  `stop`, and  `destroy`.
