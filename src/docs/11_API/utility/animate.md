...
`animate` can be accessed via Hubble's utility library

```JavaScript
Hubble._().animate();

const [animate] = Hubble.require(['animate']).from('_');
...
{
    /* Helpers */
    const [$, animate, add_event_listener] = Hubble.import(['$', 'animate', 'add_event_listener']).from('_');
