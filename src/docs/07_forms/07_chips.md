...
Chips allow users to enter information, make selections, filter content, or trigger actions and are one of Hubble's most versatile components.
...
Chips are a modifier class to Hubble's `.btn`, however as you'll see have extended functionality and states. A basic chip is created using the `.btn-chip` on `.btn`.
