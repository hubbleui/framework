...
Hubble's `FormValidator` provides a powerful module to validate and interact with your forms an easily and quickly.
...
The `showInvalid` method add the appropriate `context` class to any Hubble `.form-fields` to show them as invalid:
...
Below is an example form using the validator. In a real request, you would use Hubble's `Ajax` component, however for this example we just use a `timeout`. Check the console log for more details.
...
{
	/* Helpers */
	const [find, has_class, add_class, remove_class, on] = Hubble.import(['find', 'has_class', 'add_class', 'remove_class', 'on']).from('_');
...
/* Helpers */
const [find, has_class, add_class, remove_class, on] = Hubble.import(['find', 'has_class', 'add_class', 'remove_class', 'on']).from('Helper');
