...
Hubble provides a number of helper classes to use throughout your markup to quickly and easily apply common styling.
...
Because Hubble is mobile first, all screen-size specific grid classes will be applied to the breakpoint and above. For example a `.flex-col-md-12` class will apply to medium screen sizes and above, but not below. The grid system includes all the necessary screen-size specific classes.
...
Because flex by default spaces children evenly, Hubble has made all flex columns full-width at mobile size. You then overwrite their widths either individually or via the parent `.flex-row` at breakpoints:
...
Row gaps define the spacing between rows. With hubble's flex grid, you set this via the parent `.flex-row`.
...
Hubble's grid system comes with mobile-first fully responsive set of helpers to size row gaps according to screen size. The table below outlines the available options.
...
With Hubble's flex grid, we define alignment by option two as this just makes more sense and is simpler. With Hubble's flex grid, you set this via the parent `.flex-row` using the `.align-cols-[position]` modifier classes.
