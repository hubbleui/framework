    ...
    <div class="flex-row-fluid align-cols-center">
        <style scoped>
            .custom-list
            {
                ...
                --hb-list-item-color-active: var(--hb-brand-primary);
                --hb-list-item-color-selected: var(--hb-brand-primary);
...
$list-item-bg-active:           var(--hb-brand-info-100) !default;
$list-item-bg-selected:         var(--hb-brand-info) !default;
...
.list {
    ...
    --hb-list-item-bg-active: var(--hb-brand-info-100);
    --hb-list-item-bg-selected: var(--hb-brand-info);
...
List can be instantiated via JavaScript to generate dynamic content on the fly. To create a List dynamically, use Hubble's `Component.Create` method either via the `Hubble.Dom` or the List Component directly:
...
// Via Hibble dom
Hubble.Dom().create('List', options, container);

// Or via Component directly
Hubble.Dom().component('List').create(options, container);
```
