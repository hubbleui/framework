...
Hubble's `Pjax` Component allows you to easily handle dynamic DOM content and web-app style functionality via asynchronous requests, . 
...
```JavaScript
Hubble.require('Pjax').invoke('/foo/bar');
```
...
The Pjax Component can be accessed directly via Hubble's container using the `Pjax` key.

```JavaScript
let pjax = Hubble.Pjax();
```
...
Pjax extends Hubble's Ajax Component so the callbacks folllow the same principles. For details on callbacks, see the [Ajax Documentation](/docs/javascript/pjax/index.html#arguments)
...
|-------------------------|-------------------------------------------------------------------------------------------------------|
| `Hubble:Pjax:start`     | Sent immediately before a Pjax request is made.                                                       |
| `Hubble:Pjax:success`   | Sent after a successful Pjax request is made, DOM is updated and all assets have loaded.              |
| `Hubble:Pjax:error`     | Sent when Pjax request receives an XHR error response.                                                |
| `Hubble:Pjax:abort`     | Sent when Pjax request is interrupted and aborted by either another Pjax request or manually aborted. |

```JavaScript
window.addEventListener('Hubble:Pjax:success', (e) =>
{
